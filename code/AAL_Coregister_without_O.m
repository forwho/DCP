...
%by duanfei @ BNU
%             2013-8-27
function AAL_Coregister_without_O(folderpath,aalpath,MNI152path)
...
for SuN = 3:length(Paths)
    ...
    %second step
    matlabbatch{2}.spm.spatial.normalise.est.subj.source={rb3Dpath};
    matlabbatch{2}.spm.spatial.normalise.est.eoptions.template={MNI152path};

    %third step
    matlabbatch{3}.spm.util.defs.comp{1}.inv.comp{1}.sn2def.matname={rb3D_mat};
