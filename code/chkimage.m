function chkimage(name,allpath)
if (nargin < 1 )
...
imwrite(ABCGsall,'chkimage.png');
else
if (nargin == 2)    
Paths = dir(allpath);
cd (allpath);
mkdir chkimage;
Resultspath = strcat(allpath,'/chkimage');
for SuN = 3:length(Paths)
  temp = Paths(SuN).name;
    ddir = strcat(allpath,'/',temp);
    imagepath=strcat(ddir,'/',name);
cd (ddir);
V = spm_vol(imagepath);
Atlas = spm_read_vols(V);
orig = V.mat(1:3,4)'*(-1);
...
% ABCGsall =getABCgrayimage(ABCGsall);
cd (Resultspath);
savename = strcat(temp,'chkimage.png');
imwrite(ABCGsall,savename);
display(['finish  ' temp])
end
else
   error(message('stats:corr:TooManyInputs')); 
end
end
