function x = spm_coreg_freeze_v01(varargin)
...
% sensitive
% def_flags = struct('sep',[4 2],'params',[0 0 0  0 0 0], ...
...
for k=1:numel(VF),
    ...
    if(~isempty(find(spm_read_vols(VFk))>0))
        ...
        xk  = flags.params(:);
        for samp=flags.sep(:)',
            xk     = spm_powell_VARfreeze_poly1(xk(:), xi,sc,mfilename,VG,VFk,samp,flags.cost_fun,flags.fwhm);
            x(k,:) = xk(:)';              
            % ------- NEW CODE -------------
...
%             end
             % ------------------------------
            if (ind==2)
                trans = inv(VF.mat\spm_matrix(x(:)')*VG.mat);
                save ([Spth filesep 'mut_p' num2str(ind) '_'  Sfname '.mat'], 'trans');
            end
        end;
...
switch lower(cf)
    ...
    case 'cr',
        o = 0;
        N = VF.dim(1)*VF.dim(2)*VF.dim(3);
        float_img = spm_read_vols(VG);
        ref_img = spm_read_vols(VF);
        Y = ref_img(:);
        Y_var = sum((Y-mean(Y)).^2)/length(Y);
        for i = 1:size(H,1),
            cord_array = find(float_img == i);
            [x_cord, y_cord, z_cord] = ind2sub(VG.dim, cord_array);
            k=1;
            for  j=1:length(x_cord),
                y(k)=ref_img(x_cord(j), y_cord(j), z_cord(j));
            end
            y_mean = mean(y);
            num = length(y);
            y_var = sum((y-y_mean).^2)/num;
            o = o+(num/N*y_var);
        end
        o = o/Y_var;
    otherwise,
