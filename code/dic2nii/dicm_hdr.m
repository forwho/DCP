...
if ~isDicm && ~isTruncated % may be PAR or HEAD file
    ...
    if strcmp(ext, '.PAR') || strcmp(ext, '.REC')
        func = @philips_par;
    elseif strcmp(ext, '.HEAD') || strcmp(ext, '.BRIK')
        func = @afni_head;
    else
...
if ~fullHdr
    ...
    if ftell(fid)-s.PixelData.Start-s.PixelData.Bytes ~= 0
...
try %#ok in case of error, we return the original uint8
    ...
    for j = 1:nField
        ...
        vr = char(b(i+(1:2))); i=i+8; % also syngodt(4)
...
err = '';
if numel(fname)>4 && strcmp(fname(end+(-3:0)), '.REC')
    fname(end+(-3:0)) = '.PAR';
end
fid = fopen(fname);
...
s.PatientPosition = foo(regexp(foo, '\<.'));
...
% getTableVal('pixel spacing', 'PixelSpacing'); % bad precision
% s.PixelSpacing = s.PixelSpacing(:);
% getTableVal('slice gap', 'SpacingBetweenSlices'); % bad precision
...
if sscanf(s.SoftwareVersion, '%*c%g')>4 % guess
    x = ([s.Columns s.Rows nSL]' -1) / 2;
else
...
s.Filename = strrep(fname, '.PAR', '.REC'); % for dicm_img
